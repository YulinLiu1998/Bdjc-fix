...
extension BootomSheetChartVC{
    func Heart() {
        ...
        self.HeartCell.addSubview(chartView)
        ...
        for i in 0..<ChartData!["Content"].count {
            ...
            let str = ChartData!["Content"][i][2].stringValue
            let subStr = str.getString(startIndex: 1, count: str.count-3)
            let y = Double(subStr)
            let str1 = ChartData!["Content"][i][3].stringValue
            let subStr1 = str.getString(startIndex: 1, count: str1.count-3)
            let x = Double(subStr1)
            let entry = ChartDataEntry.init(x: x!, y: y!)
            xvalues.append(subStr)
