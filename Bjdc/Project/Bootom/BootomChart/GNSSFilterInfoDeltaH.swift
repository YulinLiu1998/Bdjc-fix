...
extension BootomSheetChartVC{
    func GNSSFilterInfoDeltaH() {
        ...
        self.GNSSFilterInfoDeltaHCell.addSubview(chartView)
        ...
        for i in 0..<ChartData!["Content"].count {
            ...
            let str = ChartData!["Content"][i][6].stringValue
            let indexPoint = str.positionOf(sub: ".")
            let subStr = str.subStringTo(endIndex: indexPoint+2)
            let y = Double(subStr)
